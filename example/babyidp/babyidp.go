...
func main() {
	...
	serviceURLstr := flag.String("service", "", "The URL to the service's SAML metadata url")
	flag.Parse()
	...
	if err != nil {
		logr.Fatalf("open users.json: %s", err)
	}
	...
	if *serviceURLstr != "" {
		go func() {
			...
			idpAddr := bind.Sniff()
			if idpAddr == "" {
				idpAddr = bind.DefaultBind
			}

			req, err := http.NewRequest("PUT", "http://"+idpAddr+"/services/service1", samlResp.Body)
			if err != nil {
